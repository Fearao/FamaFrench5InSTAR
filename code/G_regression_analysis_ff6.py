"""Run Fama-French 6-factor time-series regressions on portfolio returns."""
...
FACTOR_COLUMNS: List[str] = ["MKT_RF", "SMB", "HML", "RMW", "CMA", "WML"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=
        "Run FF6 time-series regressions (with Newey-West SEs) across 75 portfolios."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    *,
    ...
    for (ptype, size_q, factor_q), group in merged.groupby(
        ...
        stats = {
            ...
            "beta_WML": fit.params.get("WML", np.nan),
            "t_WML": fit.tvalues.get("WML", np.nan),
...
def save_outputs(results: pd.DataFrame, summary: pd.DataFrame, output_dir: Path) -> None:
    ...
    detailed_path = output_dir / "regression_results_ff6.csv"
    summary_path = output_dir / "regression_summary_ff6.csv"
...
def main() -> None:
    args = parse_args()
    factor_path = args.input_dir / "factor_returns_ff6.parquet"
