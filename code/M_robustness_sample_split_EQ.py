"""Temporal sample split robustness test for FF3+MOM_EQ."""
...
OUTPUT_FILES = {
    "period1": OUTPUT_DIR / "robustness_sample_split_EQ_period1.csv",
    "period2": OUTPUT_DIR / "robustness_sample_split_EQ_period2.csv",
}
COMPARISON_FILE = OUTPUT_DIR / "robustness_sample_split_EQ_comparison.csv"
...
def compute_period_metrics(results_df: pd.DataFrame) -> Dict[str, float]:
    ...
    metrics["alpha_sig_rate"] = (valid["p_alpha"] < SIG_LEVEL).mean() * 100

    beta_col = f"beta_{MOM_FACTOR}"
    p_col = f"p_{MOM_FACTOR}"
    mom_mask = valid[beta_col].notna()
    valid_mom = valid.loc[mom_mask]
    if len(valid_mom) > 0:
        metrics["mom_sig_rate"] = (valid_mom[p_col] < SIG_LEVEL).mean() * 100
        metrics["mean_beta_mom"] = valid_mom[beta_col].mean()
        metrics["std_beta_mom"] = valid_mom[beta_col].std(ddof=1)
...
def print_period_summary(label: str, metrics: Dict[str, float]) -> None:
    ...
    print(
        f"{MOM_FACTOR} 显著占比: {mom_sig:.1f}%"
        if not np.isnan(mom_sig)
        else f"{MOM_FACTOR} 显著占比: N/A"
    )
    print(
        f"{MOM_FACTOR} β 平均值: {mean_beta:.4f}" if not np.isnan(mean_beta) else f"{MOM_FACTOR} β 平均值: N/A"
    )
    print(
        f"{MOM_FACTOR} β 标准差: {std_beta:.4f}"
        if not np.isnan(std_beta)
        else f"{MOM_FACTOR} β 标准差: N/A"
    )
...
def main():
    ...
    beta_col = f"beta_{MOM_FACTOR}"
    beta_early = period_results[early_key][beta_col].dropna()
    beta_late = period_results[late_key][beta_col].dropna()
    ...
    print(
        f"\n{MOM_FACTOR} 显著性变化: {delta_mom_sig:.1f}pp "
        f"({early_metrics['mom_sig_rate']:.1f}% → {late_metrics['mom_sig_rate']:.1f}%)"
    )
