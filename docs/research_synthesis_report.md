...
$$
R_{i,t} - R_{f,t} = lpha_i + eta_{MKT}MKT_t + eta_{SMB}SMB_t + eta_{HML}HML_t + eta_{RMW}RMW_t + eta_{CMA}CMA_t + \epsilon_{i,t}
$$
...
$$
R_{i,t} - R_{f,t} = lpha_i + eta_{MKT}MKT_t + ... + eta_{MOM}MOM_t + \epsilon_{i,t}
$$
...
$$
R_{i,t} - R_{f,t} = lpha_i + eta' F_t + \epsilon_{i,t}
$$
...
|   排名 | 模型    |   ΔR² (pp) |   MOM显著性 (%) |   简洁性 |   经济可解释性 |   综合分 | 是否胜出   |
|-------:|:--------|-----------:|----------------:|---------:|---------------:|---------:|:-----------|
|      1 | MOM_24M |       3.22 |           41.82 |      100 |            100 |    82.55 | True       |
|      2 | MOM_4M  |       2.86 |           38.18 |      100 |            100 |    72.66 | False      |
|      3 | MOM_EQ  |       2.05 |           32.73 |       70 |             80 |    43.28 | False      |
|      4 | MOM_8M  |       1.89 |           18.18 |      100 |            100 |    42.83 | False      |
|      5 | MOM_12M |       1.59 |           34.55 |      100 |            100 |    40.36 | False      |
...
| metric                |   period1 |   period2 |   delta |
|:----------------------|----------:|----------:|--------:|
| Mean_R2               |      0.79 |      0.9  |    0.11 |
| Mean_Adj_R2           |      0.72 |      0.87 |    0.15 |
| Alpha_Sig_Rate_pct    |     24.59 |     10.53 |  -14.06 |
| MOM_Sig_Rate_pct      |     39.34 |     14.04 |  -25.31 |
| Mean_MOM_Beta         |     -0.04 |     -0.05 |   -0.01 |
| Std_MOM_Beta          |      0.56 |      0.87 |    0.3  |
| Delta_R2              |    nan    |    nan    |    0.11 |
| Delta_MOM_Sig_pct     |    nan    |    nan    |  -25.31 |
| Coefficient_Stability |    nan    |    nan    |    0.02 |
...
| window_start   | window_end   |   n_months |   mean_r2 |   mean_adj_r2 |   mom_sig_rate |   alpha_sig_rate |   mean_abs_mom_beta |
|:---------------|:-------------|-----------:|----------:|--------------:|---------------:|-----------------:|--------------------:|
| 2021-08        | 2023-07      |         24 |      0.85 |          0.82 |          45.45 |            18.18 |                0.15 |
| 2022-02        | 2024-01      |         24 |      0.87 |          0.84 |          18.42 |            15.79 |                0.18 |
| 2022-08        | 2024-07      |         24 |      0.86 |          0.84 |          25    |            29.17 |                0.18 |
| 2023-02        | 2025-01      |         24 |      0.9  |          0.87 |          44    |            12    |                0.34 |
| 2023-08        | 2025-07      |         24 |      0.92 |          0.9  |          16    |            20    |                0.3  |
...
| portfolio_type   |   period1_mom_sig |   period2_mom_sig |   delta_mom_sig |   period1_r2 |   period2_r2 |   delta_r2 |   period1_mean_abs_beta |   period2_mean_abs_beta |   stability_score |   n_portfolios_p1 |   n_portfolios_p2 |
|:-----------------|------------------:|------------------:|----------------:|-------------:|-------------:|-----------:|------------------------:|------------------------:|------------------:|------------------:|------------------:|
| size_inv         |             16    |             12    |           -4    |         0.79 |         0.92 |       0.13 |                    0.17 |                    0.22 |             17.1  |                25 |                25 |
| size_op          |             81.82 |             57.14 |          -24.68 |         0.74 |         0.78 |       0.05 |                    0.81 |                    1.93 |             29.61 |                25 |                25 |
| size_bm          |             44    |              4    |          -40    |         0.82 |         0.91 |       0.09 |                    0.23 |                    0.27 |             48.93 |                25 |                25 |
...
### 4.4 阶段5补充：MOM_24M vs MOM_EQ稳健性对比（戏剧性反转）

**研究动机**：阶段2优化中，MOM_EQ因全样本表现不如MOM_24M（ΔR²仅2.05pp vs 3.22pp，综合分43.3 vs 82.5）而被淘汰。但MOM_EQ的复杂度惩罚（-30简洁性，-20经济性）并未验证其是否通过更高稳健性来补偿。阶段3仅测试了MOM_24M，遗漏了对MOM_EQ的稳健性验证。

**补充测试**（2025-12补充）：对MOM_EQ进行完整的三维稳健性检验，并与MOM_24M对比。
...
**判定**：MOM_EQ时间稳定性显著优于MOM_24M（改善89%）

**趋势一致性（滚动窗口）**：

| 指标 | MOM_24M | MOM_EQ | **MOM_EQ改善** |
|------|---------|--------|--------------|
| 时间相关系数 | -0.37 | +0.18 | **+0.55** ✅ |
| 极差 (pp) | 29.5 | 12.0 | **-17.5pp** ✅ |
| 标准差 (pp) | 12.6 | 4.43 | **-8.2pp** ✅ |
| CV | 0.42 | 0.19 | **-0.23** ✅ |

**判定**：MOM_EQ消除了下降趋势，波动性降低65%
...
**判定**：MOM_EQ在所有类型中显著改善，特别是在size_bm中避免了灾难性崩溃

#### 表现-稳健性权衡分析

**牺牲成本**：1.17pp ΔR²（3.22% → 2.05%）

**稳健性收益**：
- 时间稳定性改善22.5pp（89%修复）
- 趋势相关系数改善0.55（从负转正）
- 类型稳健性改善36pp（size_bm避免崩溃）

**经济学解释**：
1. **多窗口分散化效应**：类似投资组合理论，降低特定窗口风险
2. **信号频率分散**：短窗口（4M）+长窗口（24M）互补，降低噪音
3. **类型异质性对冲**：不同窗口在不同投资组合类型中表现差异化

**核心发现**：**MOM_EQ通过多窗口分散化，在稳健性上全面碾压MOM_24M，改善幅度远超1.17pp ΔR²的微小损失。**

详细对比见 `docs/robustness_comparison_24M_vs_EQ.md`
...
**理由**：
- 阶段5补充测试证明MOM_EQ在稳健性上全面优于MOM_24M
- 时间稳定性改善22.5pp（89%修复）
- 类型稳健性改善36pp（size_bm避免-40pp崩溃）
- 趋势稳定性：相关系数从-0.37提升至+0.18

**操作**：
- 修改阶段2优化结论：MOM_EQ取代MOM_24M成为最优因子
- 牺牲1.17pp ΔR²换取显著的稳健性提升
- 在所有投资组合类型中统一使用MOM_EQ

**2. 修改评分体系（长期改进）**

**建议**：
- 新增稳健性作为独立评分项（权重20-30%）
- 稳健性指标包括：时间稳定性、趋势一致性、类型稳健性
- 避免全样本最优但不稳健的陷阱
...
**1. 全样本最优 ≠ 稳健有效，但多窗口分散化可同时兼顾**

- **阶段2优化（29个月全样本）**：MOM_24M以82.5分胜出，MOM_EQ仅43.3分
- **阶段3稳健性（53个月样本分割）**：MOM_24M时间稳定性崩溃（-25.3pp），类型失效（size_bm -40pp）
- **阶段5补充（2025-12）**：MOM_EQ稳健性全面碾压MOM_24M
  - 时间稳定性改善89%（-2.8pp vs -25.3pp）
  - 趋势相关系数+0.55（+0.18 vs -0.37）
  - size_bm改善36pp（-4pp vs -40pp）
- **教训**：评分体系必须纳入稳健性维度；MOM_EQ通过多窗口分散化，以微小的ΔR²损失（1.17pp）换取巨大的稳健性提升
...
1. **短期（立即执行）**：
   - **采用MOM_EQ作为最优动量因子**
   - 放弃MOM_24M（高表现但高风险）
   - 牺牲1.17pp ΔR²换取89%的时间稳定性修复和36pp的类型风险缓释
2. **中期（6-12个月）**：
   - 不同投资组合类型使用不同动量窗口
